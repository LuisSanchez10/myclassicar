<div id="seccion-vehiculo">
  ...
  <div class="row align-items-center">
    <div class="col-sm-6 col-12">
      <input id="vin" name="vin" class="inputFormVender" value="{{$publicacion->vin ?? ''}}" type="text" placeholder="Número VIN" width="100%">
    </div>
    <div class="col-sm-6 col-12"> 
      {{-- incluir el modal --}}
  ...
  <div class="row align-items-center">
    <div class="col-sm-6 col-12">
      ...
      <select id="year" name="year"  class="inputFormVender" width="100%" placeholder="Selecciona el año">
        ...
        <option value="{{$i}}" @if ($publicacion->vehicle->version->year == $i)
            selected
        @endif >{{$i}}</option>
        @endfor
      </select>
    ...
    <div class="col-sm-6 col-12">
      ...
      <select id="marcas" name="marca"  class="inputFormVender" width="100%">
        ...
        <option value="{{$marca->id}}" @if ($publicacion->vehicle->brand->id == $marca->id)
            selected
        @endif>{{$marca->brand}}</option>
        @endforeach
      </select>
    </div>
  </div>
  <div class="row align-items-center">
    <div class="col-sm-6 col-12">
      ...
      <select id="modelos" name="modelo" class="inputFormVender" width="100%">
        @foreach ($publicacion->vehicle->brand->models as $item)
          <option value="{{$item->id}}" @if ($item->id == $publicacion->vehicle->model->id)
              selected
          @endif>{{$item->model}}</option>
        @endforeach
      </select>
    </div>
    <div class="col-sm-6 col-12">
      ...
      <select id="versiones" name="version" class="inputFormVender" width="100%">
        @foreach ($publicacion->vehicle->model->versions->where('year', $publicacion->vehicle->version->year) as $item)
          <option value="{{$item->id}}" @if ($item->id == $publicacion->vehicle->version->id)
              selected
          @endif >{{$item->version}}</option>
        @endforeach
      </select>
    </div>
  </div>
