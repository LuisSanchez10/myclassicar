<div id="seccion-ficha">
  <div  class="row align-items-center">
    ...
    <div class="inputKilimetraje">
      <input id="kilometros" name="kilometros" value="{{$publicacion->kilometres}}" class="kilometros" type="number" min=0  >
...
<div class="col-sm-6 col-12">
    ...
    <input disabled id="segmento" name="segmento" class="inputFormVender" type="text"  placeholder="Tipo de vehiculo" width="100%" value ="{{$publicacion->vehicle->version->vehicle_type}}" >
  </div>
</div>
<div class="row align-items-center">
  <div class="col-sm-6 col-12">
    ...
    <select id="combustible" name="combustible" class="inputFormVender" width="100%" placeholder="Selecciona el año">
      ...
      <option value="{{$combustible->id}}"  @if ($publicacion->fuel->id == $combustible->id)
        selected
      @endif>{{$combustible->fuel}}</option>
      @endforeach
    </select>
  </div>
  <div class="col-sm-6 col-12">
    ...
    <div class="inputpar">
      <div id="transmision1" class="option-l text-center @if ($publicacion->transmission->id ==1)
          selected
      @endif">Manual</div>
      <div id="transmision2" class="option-r text-center @if ($publicacion->transmission->id ==2)
        selected
    @endif">Automática</div>
      <input type="hidden" id="transmision" name= "transmision" value ="{{$publicacion->transmission_id}}">
    </div>
  </div>
</div>
<div class="row align-items-center">
  <div class="col-sm-6 col-12">
    ...
    <select id="color" name="color" class="inputFormVender" width="100%" placeholder="Selecciona el año">
      <option value="0" selected disabled>{{$publicacion->color->color}}</option>
      @foreach ($colores as $color)
      <option value="{{$color->id}}" @if ($publicacion->color->id == $color->id)
        selected
  ...
  <div class="col-sm-6 col-12">
    ...
    <select id="interior"  name="interior" class="inputFormVender" width="100%">
      ...
      <option value="{{$interior->id}}" @if ($publicacion->car_interior->id == $interior->id)
        selected
      @endif>{{$interior->car_interior}}</option>
      @endforeach
    </select>
  </div>
</div>
<div class="row align-items-center">
  <div class="col-12">
    ...
    <textarea id="descripcion" name="descripcion" class="textareaFormVender"  width='100%' placeholder="Contanos un poco de tu clásico ..." >{{$publicacion->description}}</textarea>
  </div>
</div>
<br><br><br>
